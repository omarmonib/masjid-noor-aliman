...
export default function NewsManager({ locale }: { locale: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!titleAr.trim() || !contentAr.trim()) {
      setError(
        isAr
          ? "العنوان والمحتوى بالعربية مطلوبان"
          : "Arabic title and content are required",
      );
      return;
    }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(isAr ? "هل تريد حذف هذا الخبر؟" : "Delete this post?")) return;
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block font-arabic text-sm text-gray-600 mb-1">
                {isAr ? "المحتوى الكامل (عربي) *" : "Full Content (Arabic) *"}
              </label>
              <textarea
                value={contentAr}
            ...
            <div>
              <label className="block font-arabic text-sm text-gray-600 mb-1">
                {isAr ? "المحتوى الكامل (إنجليزي)" : "Full Content (English)"}
              </label>
              <textarea
                value={contentEn}
          ...
          <div>
            ...
            <select
              ...
              className="w-full sm:w-64 border border-gray-200 rounded-xl px-4 py-2.5 font-arabic text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
            >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="font-arabic text-lg font-bold text-gray-800">
            {isAr ? "الأخبار والإعلانات الحالية" : "Current Posts"}
          </h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-50">
            {items.map((item) => {
              const catMeta = NEWS_CATEGORIES.find(
